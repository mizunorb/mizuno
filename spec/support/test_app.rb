...
#
class TestApp # rubocop:disable ClassLength
  ...
  def call(env) # rubocop:disable AbcSize
    ...
    method = request.path[%r{^\/(\w+)}, 1]
    return(error(request, 404)) if method.nil? || method.empty?
  ...
  def push(request) # rubocop:disable MethodLength
  ...
  def rails_like_chunked(_request)
    response = Rack::Response.new
