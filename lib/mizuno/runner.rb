...
module Mizuno
  ...
  class Runner
    ...
    def self.start!
      ...
      config = (Choice.rest.first || 'config.ru')
    ...
    def self.daemonize(options)
      ...
      Runner.pid(options) && die('Mizuno is already running.')
    ...
    def self.reload(options)
      ...
      return(Runner.daemonize(options)) \
          if pid.nil? && options.delete(:restart)
    ...
    def self.stop(options)
      (pid = Runner.pid(options)) || die("Mizuno isn't running.")
    ...
    #
    def self.resolve_path(root, path)
    ...
    #
    def self.pid(options)
      options[:pidfile] || die('Speficy a --pidfile to daemonize.')
      return unless File.exist?(options[:pidfile])
    ...
    #
    def self.wait_for_server(options, timeout = 120)
    ...
    #
    def self.wait_for_server_to_die(options, timeout = 120)
      force_time_out_at = Time.now + timeout
      sleep_interval_for_next_retry = 0.1
    ...
    def self.connect_to_server_as_client(server_options, timeout)
    ...
    #
    def self.die(message, success = false)
      $stderr.puts(message)
      exit(success ? 0 : 1)
    end
  end
