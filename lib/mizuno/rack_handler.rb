...
module Mizuno
  ...
  class RackHandler < AbstractHandler
    ...
    #
    def handle(_target, _base_request, request, response)
      handle_exceptions(response) do
        ...
        env['async.callback'] = proc do |rack_response|
    ...
    def rack_to_servlet(rack_response, response)
      ...
      finished = (headers.empty? && \
          body.respond_to?(:empty?) && body.empty?)
      return(true) if finished && response.isCommitted
